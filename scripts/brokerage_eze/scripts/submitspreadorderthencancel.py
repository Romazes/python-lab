...
class CreateSpreadOrder:
    def __init__(self):
        ...
        self.leg2_orderid = ''
        self.status = ''
        self.submitthencancelorder = True  # When True cancel script is executed when order goes LIVE.
        ...
        while True:
            try:
                if subscribe_response:
                    ...
                    # To check if Order is Live and send cancel Order
                    if self.parent_orderid and self.leg1_orderid and self.leg2_orderid and \
                            self.status != State.CancelPending.name and self.currentOrder and self.submitthencancelorder:
                        if order_info.Type == 'UserSubmitCompoundOrder' and order_info.CurrentStatus == 'LIVE':
                            print("ORDER LIVE -- SUBMITTING CANCEL")
                            self.cancel_order()
    ...
    def send_spread_order(self):
        ...
        ord_request.SpreadNumLegs.value = 2  # Number of Legs in the spread
    ...
    def cancel_order(self):
        ...
        cancel_spreadorder = ord.CancelPairOrderRequest(OrderId=self.parent_orderid,  # pair order id
                                                        UserToken=self.xapiLib.userToken)
        self.status = State.CancelPending.name
        resp_cancel = self.xapiLib.get_order_service_stub().CancelPairOrder(cancel_spreadorder)
